  ...
  if (path != NULL)
    {
      ...
      char *app_id = glib_lookup_sandboxed_app_id (&local_error);

      if (app_id == NULL)
        {
          g_warning ("Can't determine app ID: %s", local_error->message);
          g_propagate_error (error, local_error);
          return FALSE;
        }

      real_uri = g_document_portal_register_document (uri, app_id, &local_error);
      g_free (app_id);
